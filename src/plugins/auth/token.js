...
function tokenAuth(manager, client, user, authToken, callback) {
	...
	if (!client.config.authToken) {
		log.error(`User ${colors.bold(user)} with no auto token set tried to sign in.`);
		return callback(false);
	}
	
	 
	const matching = client.config.authToken === authToken;
	
	client.config.authToken = false;  //tokens are single use
